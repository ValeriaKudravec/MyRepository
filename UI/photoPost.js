var photoPosts = [
  ...
  (function() {
      ...
      function getAuthorPost(skip, top, filterConfig){
            ...
            var index = 0;
                  ...
                  for(i = 0;i < photoPosts.length; ++i){
                        if(photoPosts[i].author == filterConfig.author){
                              ...
                              for(k = 0; k < photoPosts[i].hashtag.length; k++){
                              if(photoPosts[i].hashtag[k] == filterConfig.hashtag){
                                    arr[index]=photoPosts[i];
                                    ++index;
                              }
                        }
            ...
            for(l = skip; l < top; ++l){
                  returnArray[l] = arr[l];
                  alert(returnArray[l].descriprion);
            }
            returnArray.sort(comparePost);
            return returnArray;
      }
      if(top+skip > photoPosts.length){
            top = arr.length;
      }
      for(l = skip; l < top; ++l){
            returnArray[l] = photoPosts[l];
      }
      ...
      alert(getPhotoPost('1').photoLink);
     getAuthorPost(0, 10);
